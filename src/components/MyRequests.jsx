...
const MyRequests = () => {
  ...
  const [services, setServices] = useState([]); // State for services offered
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const servicesQuery = query(
        ...
        where('userId', '==', user.uid) // Assuming services have a userId field
      );
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-2xl font-bold text-blue-600 mb-6">My Requests</h2>
        ...
        {loading ? (
          ...
          <>
                ...
                <ul className="space-y-4">
                  {sentRequests.map((req) => (
                    <li key={req.id} className="p-4 bg-gray-100 rounded shadow">
                      <p><strong>Service Title:</strong> {req.serviceTitle}</p>
              ...
              {receivedRequests.length === 0 ? (
                ...
                <ul className="space-y-4">
                  {receivedRequests.map((req) => (
                    <li key={req.id} className="p-4 bg-blue-50 rounded shadow">
                      <p><strong>Service Title:</strong> {req.serviceTitle}</p>
                      <p><strong>From User:</strong> {req.fromUserId}</p>
                      <p><strong>Status:</strong> {req.status}</p>
            ...
            {/* Offered Services */}
            <div className="mt-10">
              <h3 className="text-xl font-semibold mb-4">My Offered Services</h3>
              {services.length === 0 ? (
                <p className="text-gray-500">No services offered yet.</p>
              ) : (
                <ul className="space-y-4">
                  {services.map((service) => (
                    <li key={service.id} className="p-4 bg-yellow-50 rounded shadow">
